...
extern UART_HandleTypeDef huart2;          // optional, for printf over UART

// ------------------- DHCX register map (subset) -------------------
...
#define DHCX_REG_OUTX_L_G   0x22   // Gx..Gz (6B) then Ax..Az (6B) contiguously
#define DHCX_REG_OUTX_L_A   0x28
#define DHCX_REG_WAKE_UP_SRC   0x1B

//------codex------------------
#define DHCX_ODR_104        (0x04u)
#define DHCX_ODR_208        (0x05u)
#define DHCX_ODR_416        (0x06u)
...
#define DHCX_CTRL3_IF_INC      (1u << 2)
#define DHCX_CTRL3_PP_OD       (1u << 4)   // 1=open-drain, 0=push-pull
#define DHCX_CTRL3_H_LACTIVE   (1u << 5)   // 1=active-low, 0=active-high
...
// CTRL3_C bits we’ll use
#define DHCX_BDU            (1u<<6)
#define DHCX_IF_INC         (1u<<2)

// Choose ODR/FS codes (typical ST mapping; adjust if needed)
#define DHCX_ODR_6k66       (0x0Au)  // many ST IMUs: 0x0A -> 6.66 kHz
#define DHCX_ODR_1k66       (0x08u)
#define DHCX_ODR_833        (0x07u)

#define DHCX_FS_XL_2G       (0x00u<<2)  // CTRL1_XL FS bits
#define DHCX_FS_G_2000DPS   (0x03u<<2)  // CTRL2_G  FS bits
...
#define WIN_SAMPLES              256u

#define TRIG_THR_ON_COUNTS       500u   // safely above OFF (~50), below LOW (~850)
#define TRIG_THR_OFF_COUNTS      150u   // hysteresis: below this means “really off”

#define TRIG_ON_REQUIRED_WINS    6u     // ~0.92s sustained ON
#define TRIG_OFF_REQUIRED_WINS   8u     // ~1.23s sustained OFF

#define TRIG_SPIKE_REJECT_COUNTS  1600u   // ignore big spikes when OFF
#define TRIG_KEEP_MAX_JUMP        400u    // stability for “real fan”, tune 300-600
#define TRIG_ON_MAX_SCORE         12u
#define TRIG_OFF_MAX_SCORE        12u

#define TRIG_ON_SCORE_TO_ON       8u      // harder to turn on
#define TRIG_OFF_SCORE_TO_OFF     5u      // easier to turn off

#define FAN_ON_THR               500u
#define FAN_OFF_THR              150u
#define FAN_CONFIRM_WINDOWS      3u
#define FAN_CONFIRM_MIN_HITS     2u
...
// MD1_CFG bits (Table shows order; commonly bit5 = INT1_WU) :contentReference[oaicite:9]{index=9}
#define MD1_INT1_WU       (1U << 5)

// WAKE_UP_DUR fields (see Table 131) :contentReference[oaicite:10]{index=10}
#define WAKE_THS_W_BIT    (1U << 4)      // 0: FS/2^6, 1: FS/2^8
#define WAKE_DUR_MASK     (3U << 5)      // bits 6:5
#define SLEEP_DUR_MASK    (0x0FU)        // bits 3:0
...
static uint64_t  sum_setup = 0, sum_dma = 0, sum_post = 0, sum_total = 0;
static uint32_t  min_total = 0xFFFFFFFFu, max_total = 0;

// keep the latest sample to print
static int32_t last_gx_mdps, last_gy_mdps, last_gz_mdps;
static int32_t last_ax_mg,   last_ay_mg,   last_az_mg;
...
typedef struct { int16_t gx, gy, gz, ax, ay, az; } dhcx_raw_t;


// ---- SPI2 timing stamps ----
static volatile uint32_t t_spi2_start = 0;
static volatile uint32_t t_spi2_callret = 0;
static volatile uint32_t t_spi2_cb = 0;
static volatile uint32_t t_spi2_done = 0;
...
static volatile uint8_t imu_wakeup_event = 0;
...
static det_state_t det_state = DET_LEARNING;
static uint32_t learn_count = 0;
static uint8_t last_similarity = 100;
...
static void LED_ShowSleep(void)
{
    HAL_GPIO_WritePin(LED_HEALTHY_GPIO_Port, LED_HEALTHY_Pin, GPIO_PIN_RESET);
    HAL_GPIO_WritePin(LED_ANOM_GPIO_Port,    LED_ANOM_Pin,    GPIO_PIN_SET);
}
...
static void process_window(void)
{
    ...
    int n = snprintf(line, sizeof(line),
                     "NEAI state=%c sim=%u learn=%lu\r\n",
                     state_char, similarity, (unsigned long)learn_count);
...
{
  uint8_t b[6];
  HAL_StatusTypeDef st = dhcx_read_dma(DHCX_REG_OUTX_L_A, b, 6);
...
/**
 * Compute vibration metric over N samples:
 * - reads accel only
 * - returns MAD_total = MADx + MADy + MADz  (all in raw counts)
 */
static uint32_t calc_mad_total_counts(uint16_t N)
{
  ...
  for (uint16_t i = 0; i < N; i++)
  {
    ...
    if (HAL_OK != dhcx_read_accel_dma(&ax, &ay, &az)) {
      // if a read fails, just try again next iteration
      i--;
      continue;
    }
    ...
    // simple pacing for monitor mode; adjust for your ODR later
    // (for now keep it stable, not precise)
    HAL_Delay(5);
  }
  ...
  // 2) Second pass: mean absolute deviation
  ...
  for (uint16_t i = 0; i < N; i++)
  {
    ...
    madx += (uint32_t)iabs32((int32_t)ax - meanx);
    mady += (uint32_t)iabs32((int32_t)ay - meany);
    madz += (uint32_t)iabs32((int32_t)az - meanz);
...
static void debug_print_vibration_window_counts(uint16_t N)
{
  uint32_t mad_total = calc_mad_total_counts(N);

  char line[96];
  int n = snprintf(line, sizeof(line),
                   "MON N=%u MAD_total=%lu (counts)\r\n",
                   (unsigned)N, (unsigned long)mad_total);
...
static void dhcx_init_monitor_mode(void)
{
  // BDU + auto-increment
  dhcx_write(DHCX_REG_CTRL3_C, DHCX_BDU | DHCX_IF_INC);

  // Gyro power-down: ODR = 0 (keep FS bits whatever, not important if off)
  dhcx_write(DHCX_REG_CTRL2_G, 0x00);

  // Accel: ODR = 208 Hz (or 416), FS=2g
  dhcx_write(DHCX_REG_CTRL1_XL, (uint8_t)((DHCX_ODR_208 << 4) | DHCX_FS_XL_2G));
}
...
static uint8_t fan_confirmed = 0;
static uint8_t off_cnt = 0;
static uint32_t last_mad = 0;



static uint8_t on_score  = 0;
static uint8_t off_score = 0;
...
static void enter_monitor_mode(void)
{
  // 1) Reduce ODR to save power (example: 208 Hz or 416 Hz)
  // CTRL1_XL: ODR bits in [7:4] typically
  dhcx_write(DHCX_REG_CTRL1_XL, (uint8_t)((DHCX_ODR_208 << 4) | DHCX_FS_XL_2G));

  // 2) Enable wake-up interrupt in IMU (register details depend on DHCX)
  // You need to configure:
  // - WAKE_UP_THS (threshold)
  // - WAKE_UP_DUR (duration)
  // - MD1_CFG / MD2_CFG route wake-up to INT1/INT2
  // This is the only “sensor-specific” part.

  //dhcx_enable_wakeup_int1(/*ths*/, /*dur*/);
...
{
  // High ODR for real processing
  dhcx_write(DHCX_REG_CTRL1_XL, (uint8_t)((DHCX_ODR_1k66 << 4) | DHCX_FS_XL_2G));

  // Optionally disable wake-up interrupt (not needed while active)
  //dhcx_disable_wakeup_int1();
...
static void dhcx_set_xl_odr(uint8_t odr_code)
{
  // CTRL1_XL = [ODR_XL(7:4)] [FS_XL(3:2)] [LPF2_XL(1)] [0]
  uint8_t v = (uint8_t)((odr_code << 4) | DHCX_FS_XL_2G);
  dhcx_write(DHCX_REG_CTRL1_XL, v);
}

static void dhcx_set_xl_odr_lowpower_104(void)
{
  dhcx_set_xl_odr(DHCX_ODR_104);
}

static void dhcx_set_xl_odr_run_1k66(void)
{
  dhcx_set_xl_odr(DHCX_ODR_1k66);
}
...
static uint8_t dhcx_read1(uint8_t reg)
{
  uint8_t v = 0;
  dhcx_read_dma(reg, &v, 1);
  return v;
}
...
static void dhcx_config_wakeup_int1(uint8_t wk_ths, uint8_t wake_dur, uint8_t ths_weight_sel)
{
	 // 1) Turn on interrupt functions: TAP_CFG2.INTERRUPTS_ENABLE = 1
	  dhcx_write(REG_TAP_CFG2, 0x80);

	  // 2) Optional but recommended: select filter + latch behavior
	  // ST example uses 0x51 (latched + HPF selection). :contentReference[oaicite:3]{index=3}
	  dhcx_write(REG_TAP_CFG0, 0x51);

	  // 3) Threshold
	  dhcx_write(REG_WAKE_UP_THS, (uint8_t)(wk_ths & 0x3F));

	  // 4) Duration + threshold weight
	  uint8_t dur = (uint8_t)((wake_dur & 0x03) << 5);
	  if (ths_weight_sel) dur |= WAKE_THS_W_BIT;
	  dhcx_write(REG_WAKE_UP_DUR, dur);

	  // 5) Route wake-up to INT1
	  uint8_t md1 = dhcx_read1(REG_MD1_CFG);
	  md1 |= MD1_INT1_WU;
	  dhcx_write(REG_MD1_CFG, md1);

	  // If latched mode is enabled, remember: read WAKE_UP_SRC to clear it. :contentReference[oaicite:4]{index=4}
	}
...
static void enter_stop2(void)
{
  ...
  // Suspend SysTick to avoid waking from it
  HAL_SuspendTick();

  // Enter STOP2, wake on interrupt
...
static void restore_clocks_after_stop2(void)
{
  SystemClock_Config();
}
...
static volatile uint32_t exti_wake_cnt = 0;
static volatile uint16_t exti_last_pin = 0;
static volatile uint8_t exti_last_level = 0;
static volatile uint32_t exti_last_tick = 0;


void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  if (GPIO_Pin == INT1_DHCX_Pin) {
    wake_event = 1;
    exti_wake_cnt++;
    exti_last_pin = GPIO_Pin;
    exti_last_level = (uint8_t)HAL_GPIO_ReadPin(INT1_DHCX_GPIO_Port, INT1_DHCX_Pin);
    exti_last_tick = HAL_GetTick();
  }
}
...
// placeholder: FIFO not used yet in your current working approach
static void dhcx_fifo_enable_watermark(uint16_t wtm_samples)
{
  (void)wtm_samples; // avoid unused warning
  // do nothing for now
}

// run mode: put your existing “system_on==1” processing here
static void run_processing(void)
{
  // MIN CHANGE:
  // Move the current content of your "else { ... }" (the active part)
  // into this function.
  //
  // For example, if your active loop does:
  //  - read accel samples
  //  - build window (256)
  //  - compute MAD_total
  //  - update on/off scores
  // keep all of that here.

  // Example skeleton (replace with your current code):
  // if (dhcx_both_ready()) {
  //   dhcx_raw_t r;
  //   if (HAL_OK == dhcx_read_gyro_accel_dma(&r)) {
  //     // use r.ax,r.ay,r.az for window buffering + MAD_total logic
  //   }
  // }
}
...
static void wake_debug_poll_10s(void)
{
  uint8_t src_prev = 0xFF;
  GPIO_PinState pin_prev = (GPIO_PinState)2; // invalid start
  uint32_t t0 = HAL_GetTick();

  uart2_printf("Wake debug poll (10s)... shake now\r\n");

  while ((HAL_GetTick() - t0) < 10000)
  {
    // 1) Read INT1 pin state (PB8)
    GPIO_PinState pin = HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_8);
    if (pin != pin_prev)
    {
      pin_prev = pin;
      uart2_printf("PB8(INT1)=%d\r\n", (int)pin);
    }

    // 2) Read WAKE_UP_SRC (event status inside IMU)
    uint8_t src = dhcx_read1(DHCX_REG_WAKE_UP_SRC);

    if (src != src_prev)
    {
      src_prev = src;
      uart2_printf("WAKE_UP_SRC=0x%02X\r\n", src);
    }
...
static void dhcx_force_int_pushpull_activehigh(void)
{
  uint8_t v = dhcx_read1(DHCX_REG_CTRL3_C);

  v |= DHCX_CTRL3_IF_INC;
  v &= (uint8_t)~(DHCX_CTRL3_PP_OD | DHCX_CTRL3_H_LACTIVE);

  dhcx_write(DHCX_REG_CTRL3_C, v);
}
...
int main(void)
{
 ...
 // SystemPower_Config();
  ...
  dhcx_init_monitor_mode();
  HAL_UART_Transmit(&huart2, (uint8_t*)"Monitor mode ON\r\n", 17, HAL_MAX_DELAY);
  ...
  // Initialize NanoEdge AI
   enum neai_state neai_ret = neai_anomalydetection_init();
   if (neai_ret != NEAI_OK) {
       char msg[64];
       int n = snprintf(msg, sizeof(msg), "NEAI init error: %d\r\n", neai_ret);
       HAL_UART_Transmit(&huart2, (uint8_t*)msg, n, HAL_MAX_DELAY);
   } else {
       HAL_UART_Transmit(&huart2, (uint8_t*)"NEAI init OK\r\n", 14, HAL_MAX_DELAY);
   }
  ...
  while (1)
  {
	  ...
	  if (!system_on)
	    {
	      // IMU in monitor mode: low ODR + wake-up enabled
		  dhcx_set_xl_odr_lowpower_104();        // example: 104 Hz (no floats)
	      dhcx_config_wakeup_int1(/*wk_ths=*/2, /*wake_dur=*/1, /*ths_weight_sel=*/1);

	      wake_event = 0;

	      // Clear any previous latched wake-up source (important if latched mode is enabled)
	      (void)dhcx_read1(DHCX_REG_WAKE_UP_SRC);

	      // (Optional) clear EXTI pending just in case (good hygiene)
	      // __HAL_GPIO_EXTI_CLEAR_IT(INT1_DHCX_Pin);

	      LED_ShowSleep();
	      HAL_UART_Transmit(&huart2, (uint8_t*)"GO STOP2\r\n", 10, HAL_MAX_DELAY);

	      {
	        uint8_t pin_level = (uint8_t)HAL_GPIO_ReadPin(INT1_DHCX_GPIO_Port, INT1_DHCX_Pin);
	        char dbg[96];
	        int n_dbg = snprintf(dbg, sizeof(dbg),
	                             "PRE-STOP INT1=%u exti_cnt=%lu\r\n",
	                             (unsigned)pin_level, (unsigned long)exti_wake_cnt);
	        HAL_UART_Transmit(&huart2, (uint8_t*)dbg, (uint16_t)n_dbg, HAL_MAX_DELAY);
	      }
...

	      {
	        uint8_t pin_level = (uint8_t)HAL_GPIO_ReadPin(INT1_DHCX_GPIO_Port, INT1_DHCX_Pin);
	        if (pin_level) {
	          // If INT1 is already high, treat it as a wake to avoid sleeping forever.
	          wake_event = 1;
	        } else {
	          wake_event = 0;
	          __HAL_GPIO_EXTI_CLEAR_IT(INT1_DHCX_Pin);
	          HAL_SuspendTick();
	          enter_stop2();                  // <-- NOW you really sleep here
	        }
	      }

	      restore_clocks_after_stop2();    // <-- Restore clocks BEFORE UART/SPI prints
	      LED_ShowHealthy();
	      ...
	      HAL_UART_Transmit(&huart2, (uint8_t*)msg, (uint16_t)n, HAL_MAX_DELAY);
	      {
	        ...
	        char dbg[120];
	        int n_dbg = snprintf(dbg, sizeof(dbg),
	                             "POST-WAKE INT1=%u exti_cnt=%lu last_pin=%u last_lvl=%u last_tick=%lu\r\n",
	                             (unsigned)pin_level,
	                             (unsigned long)exti_wake_cnt,
	                             (unsigned)exti_last_pin,
	                             (unsigned)exti_last_level,
	                             (unsigned long)exti_last_tick);
	        HAL_UART_Transmit(&huart2, (uint8_t*)dbg, (uint16_t)n_dbg, HAL_MAX_DELAY);
	      }
	      ...
	      if (wake_event) {
	        ...
	        if (hits >= FAN_CONFIRM_MIN_HITS) {
	          ...
	          fan_confirmed = 0;
	          system_on = 0;
	          HAL_UART_Transmit(&huart2, (uint8_t*)"FALSE WAKE -> STOP2\r\n", 22, HAL_MAX_DELAY);
	          wake_event = 0;
	          LED_ShowSleep();
	          continue;
	        }
	      ...
	      if (fan_confirmed) {
	        if (dhcx_both_ready()) {
	          ...
	          if (HAL_OK == dhcx_read_gyro_accel_dma(&r)) {
	            ...
	            neai_buffer[neai_sample_idx * AXIS_NUMBER + 1] = ay_mg;
...
/*
		  ...
		  char line[120];
		  int n = snprintf(line, sizeof(line),
		                   "MAD=%lu on_thr=%u off_thr=%u on_cnt=%u off_cnt=%u sys=%u\r\n",
		                   (unsigned long)mad,
		                   (unsigned)TRIG_THR_ON_COUNTS,
		                   (unsigned)TRIG_THR_OFF_COUNTS,
		                   (unsigned)on_score,
		                   (unsigned)off_score,
		                   (unsigned)system_on);
		  HAL_UART_Transmit(&huart2, (uint8_t*)line, n, HAL_MAX_DELAY);
		  ...
		  uint32_t diff = (mad > last_mad) ? (mad - last_mad) : (last_mad - mad);

		  // “stable_high” means: looks like a running fan, not a random thump
		  uint8_t stable_high = (mad >= TRIG_THR_ON_COUNTS) &&
		                        (mad <= TRIG_SPIKE_REJECT_COUNTS) &&
		                        (diff <= TRIG_KEEP_MAX_JUMP);
		  ...
		  if (!system_on)
		  {
			  ...
			  // Go to STOP until interrupt occurs

			  HAL_UART_Transmit(&huart2, (uint8_t*)"Entering STOP...\r\n", 18, HAL_MAX_DELAY);

			  // HARDER to turn ON:
		    // - reject huge spike from OFF baseline
		    // - require stable_high evidence to accumulate score
		    if (last_mad < TRIG_THR_OFF_COUNTS && mad > TRIG_SPIKE_REJECT_COUNTS) {
		      // impact spike → do not accumulate
		      if (on_score > 0) on_score--;
		    }
		    else if (stable_high) {
		      if (on_score < TRIG_ON_MAX_SCORE) on_score++;
		    } else {
		      // decay faster when not convincing
		      on_score = (on_score >= 2) ? (on_score - 2) : 0;
		    }

		    if (on_score >= TRIG_ON_SCORE_TO_ON) {
		      system_on = 1;
		      on_score = 0;
		      off_score = 0;
		      LED_ShowHealthy();
		      HAL_UART_Transmit(&huart2, (uint8_t*)"TRIGGER ON\r\n", 12, HAL_MAX_DELAY);
		    }
		  }
		  else
		  {
		    // EASIER to turn OFF:
		    // - accumulate OFF score quickly when MAD is low
		    // - DO NOT reset OFF score because of one shake
		    if (mad <= TRIG_THR_OFF_COUNTS) {
		      if (off_score <= TRIG_OFF_MAX_SCORE - 2) off_score += 2;  // fast off
		      else off_score = TRIG_OFF_MAX_SCORE;
		    }
		    else if (stable_high) {
		      // fan clearly running → gently reduce off_score
		      if (off_score > 0) off_score--;
		    }
		    else {
		      // ambiguous (random shakes / medium values) → keep off_score as-is
		      // (key point: no reset)
		    }

		    if (off_score >= TRIG_OFF_SCORE_TO_OFF) {
		      system_on = 0;
		      on_score = 0;
		      off_score = 0;
		      LED_AllOff();
		      HAL_UART_Transmit(&huart2, (uint8_t*)"TRIGGER OFF\r\n", 13, HAL_MAX_DELAY);
		    }
		  }
...
// ---------- DMA completion flags / callbacks ----------
extern SPI_HandleTypeDef hspi2;
...
// ---------- ISM330DHCX registers (must match your PD defines) ----------
...
static void dhcx_init(void){
  dhcx_write(DHCX_REG_CTRL3_C, DHCX_BDU | DHCX_IF_INC);
  dhcx_write(DHCX_REG_CTRL2_G,  (uint8_t)((DHCX_ODR_1k66<<4) | DHCX_FS_G_2000DPS));
  dhcx_write(DHCX_REG_CTRL1_XL, (uint8_t)((DHCX_ODR_1k66<<4) | DHCX_FS_XL_2G));
}
