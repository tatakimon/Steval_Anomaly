...
/* >>> USER PATCH: quiet-window + handle mirror (from app_blesensorspnpl.c) <<< */
extern volatile uint8_t user_gatt_ready;
extern uint32_t user_conn_t0;
extern uint16_t ConnectionHandle;     /* defined in app_blesensorspnpl.c */
/* <<< USER PATCH END >>> */
...
/* >>> USER PATCH: extern USER DATA handles/flags <<< */
extern uint16_t userDataSvcHandle;
extern uint16_t userDataCharHandle;
extern volatile uint8_t user_data_cccd;
...
end_label:
  ...
  /* >>> USER PATCH: helper to stop advertising (BlueNRG-MS / M2) <<< */
  void user_stop_advertising(void)
  {
    ble_status_t ret;

    /* For BlueNRG-MS / BlueNRG-2, this is the usual API to stop advertising */
    ret = aci_gap_set_non_discoverable();

    BLE_MANAGER_PRINTF("USER[RADIO]: ADV off (sleep). rs=%d\r\n", ret);
  }
  /* <<< USER PATCH END >>> */
...
/**
  ...
  */
  ...
  /* >>> USER RADIO PATCH: direct ADV start helper <<< */
  void set_connectable_ble(void)
  {
    ble_status_t ret;
    Advertising_Set_Parameters_t advertising_set_parameters[1];
    static uint8_t adv_configured = 0;   /* >>> USER PATCH: run config only once <<< */

    /* >>> USER PATCH: configure advertising only on first call <<< */
    if (adv_configured == 0U)
    {
      /* Build advertising data (unchanged from your code) */
      manuf_data[0 ] = 0x02;
      ...
      /* Set the Custom BLE Advertise Data (ST hook, unchanged) */
      ble_set_custom_advertise_data(manuf_data);

      set_connectable_function(manuf_data);

      /* Configure advertising set (only once) */
      ...
      ret = aci_gap_set_advertising_data_nwk(0, ADV_COMPLETE_DATA,
                                             BLE_MANAGER_ADVERTISE_DATA_LENGHT, manuf_data);
      if (ret != BLE_STATUS_SUCCESS)
      {
      ...
      adv_configured = 1U;  /* <<< mark config done >>> */
    }
    /* <<< USER PATCH END (config only once) >>> */

    /* Now (every call) we can enable ADV with handle 0 */
                                      ...
                                      uint8_t master_clock_accuracy)
{
  ...
  /* >>> USER PATCH: start quiet window + mirror handle <<< */
  user_gatt_ready = 0;
  user_conn_t0    = HAL_GetTick();
  ConnectionHandle = connection_handle;   // keep both in sync for our gating
  /* <<< USER PATCH END >>> */
  ...
  /* >>> USER RADIO PATCH: reset advertising duty-cycle on connection <<< */
  // extern uint8_t user_radio_state;
  // user_radio_state = 0U;
   /* <<< USER RADIO PATCH END >>> */
                                      ...
                                      uint8_t reason)
{
  ...
  /* >>> USER PATCH: not ready when disconnected + mirror handle <<< */
    user_gatt_ready = 0;
    ConnectionHandle = 0;
  /* <<< USER PATCH END >>> */
                                       ...
                                       uint8_t attr_data[])
{
  ...
  /* USER PATCH: handle CCCD for USER DATA */
  if (userDataCharHandle != 0U && attr_handle == (uint16_t)(userDataCharHandle + 2U))
  {
    if (attr_data_length >= 2U) {
      uint8_t cccd = attr_data[0];
      user_data_cccd = (cccd & 0x01U) ? 1U : 0U;   // bit0 = Notify
      BLE_MANAGER_PRINTF("USER: DATA notify %s\r\n",
                         user_data_cccd ? "ENABLED" : "DISABLED");
    }
    found_handle = 1U;   // <<< IMPORTANT: tell the function we handled this
  }
