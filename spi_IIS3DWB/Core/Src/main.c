...
// ---- ISM330DHCX CS ----
#define IMU_CS_LOW()   HAL_GPIO_WritePin(CS_DHCX_GPIO_Port, CS_DHCX_Pin, GPIO_PIN_RESET)
#define IMU_CS_HIGH()  HAL_GPIO_WritePin(CS_DHCX_GPIO_Port, CS_DHCX_Pin, GPIO_PIN_SET)
...
static inline HAL_StatusTypeDef ism_write(uint8_t reg, const uint8_t *data, uint16_t len)
{
...
static inline HAL_StatusTypeDef ism_read(uint8_t reg, uint8_t *data, uint16_t len)
{
    HAL_StatusTypeDef st;
...
#define CTRL4_C_I2C_DIS  (1u << 2)

/* ±2 g sensitivity: 0.061 mg/LSB */
#define MG_PER_LSB_x1000 61   // mg * 1000
...
/* Optional: quick I2C scanner to discover the live address */
...
int main(void)
{
  ...
  /* Set measurement mode.
     Simplest is “one-shot” (start a conversion when we read),
     or select a low ODR in CTRL to run continuously.
     Below example enables “continuous” by setting ODR bits (datasheet mapping):
     If your part uses different bitfields, this write is safe to skip. */
  ...
  /* ===== Init ISM330DHCX (SPI Mode 3) ===== */
  DHCX_CS_HIGH();
  spi2_set_mode3();

  uint8_t who_ism = 0;
  ( void ) ism_read(DHCX_WHO_AM_I, &who_ism, 1);   /* expect 0x6B */

  /* BDU=1, IF_INC=1 */
  uint8_t v = 0x44;
  ( void ) ism_write(DHCX_CTRL3_C, &v, 1);

  /* Accel 104 Hz, ±2 g */
  v = 0x40;  (void) ism_write(DHCX_CTRL1_XL, &v, 1);

  /* Gyro 104 Hz (optional) */
  v = 0x40;  (void) ism_write(DHCX_CTRL2_G, &v, 1);

  HAL_Delay(5);

  /* Return SPI2 to IIS3DWB mode so your existing loop continues unchanged */
  spi2_set_mode0();



  // Bind devices on SPI2 (adjust the CS names to your project’s pins)
  spi_dev_t DWB = { .spi=&hspi2, .cs_port=CS_DWB_GPIO_Port, .cs_pin=CS_DWB_Pin };
  spi_dev_t ISM = { .spi=&hspi2, .cs_port=CS_ISM_GPIO_Port, .cs_pin=CS_ISM_Pin }; // <-- use your ISM CS pin macro
	  ...
	  while (1)
	  {
	      ...
	      if (iis3dwb_read(REG_STATUS, &st, 1) == HAL_OK && (st & 0x01))
	      {
	          ...
	          if (iis3dwb_read(REG_OUTX_L, d, 6) == HAL_OK)
	          {
	              ...
	              uint8_t tbuf[2];
	              int32_t t_centi = 0;
	              if (HAL_I2C_Mem_Read(&hi2c2, stts_addr, 0x06, I2C_MEMADD_SIZE_8BIT, tbuf, 2, 50) == HAL_OK)
	              {
	      ...
	      uint8_t s2 = 0;
	      if (ism_read(DHCX_STATUS, &s2, 1) == HAL_OK && (s2 & 0x01))   // XLDA
	      {
	          uint8_t b[6];
	          if (ism_read(DHCX_OUTX_L_A, b, 6) == HAL_OK)
	          {
