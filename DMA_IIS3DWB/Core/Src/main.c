...
#define MAX_XFER  (1 + 32)   // 1 addr + up to 32 data bytes


// ---- SPI2 sensor CS pins ----
#define CS_DWB_GPIO_Port       GPIOF    // IIS3DWB CS
#define CS_DWB_Pin             GPIO_PIN_12

#define CS_DHCX_GPIO_Port      GPIOH    // ISM330DHCX CS  (keep HIGH)
#define CS_DHCX_Pin            GPIO_PIN_15

#define CS_IIS2DLPC_GPIO_Port  GPIOH    // IIS2DLPC CS    (keep HIGH, if present)
#define CS_IIS2DLPC_Pin        GPIO_PIN_6
...
static uint8_t  spi_tx[MAX_XFER];
static uint8_t  spi_rx[MAX_XFER];
static volatile uint8_t spi_dma_done = 0;
static const spi_dev_t *spi_dma_dev = NULL;   // which device's CS to release in callbacks
...
static void cs_low (const spi_dev_t *d);
static void cs_high(const spi_dev_t *d);
static void cfg_output_high(GPIO_TypeDef* port, uint16_t pin);
static void Bus_Quiesce_All_CS(void);
...
HAL_StatusTypeDef dev_read_dma (const spi_dev_t *d, uint8_t reg, uint8_t *data, uint16_t len);
static void iis3dwb_start(const spi_dev_t *d);
static void iis3dwb_dump_regs(const spi_dev_t *d);
...
// ---- Blocking SPI register I/O (Mode-0 friendly) ----
static HAL_StatusTypeDef dev_write(const spi_dev_t *d, uint8_t reg, const uint8_t *data, uint16_t len)
{
...
static HAL_StatusTypeDef dev_read(const spi_dev_t *d, uint8_t reg, uint8_t *data, uint16_t len)
{
    HAL_StatusTypeDef st;
    uint8_t cmd = reg | 0x80u; // RW=1
    cs_low(d);
...
{
    // 1) Software reset
    uint8_t v = 0x01;                     // CTRL3_C.SW_RESET
    dev_write(d, 0x12, &v, 1);            // CTRL3_C = 0x12
    HAL_Delay(5);

    // 2) Interface/config
    v = (1u<<6) | (1u<<2);                // BDU=1, IF_INC=1
    dev_write(d, 0x12, &v, 1);            // CTRL3_C

    v = (1u<<2);                          // I2C_disable=1 (4-wire SPI only)
    dev_write(d, 0x13, &v, 1);            // CTRL4_C = 0x13

    // 3) Enable accel/ODR (Mode-0, prescaler /32 is fine)
    // ODR and FS live in CTRL1_XL (0x10). 0xA0 = high ODR, ±2g.
    v = 0xA0;
    dev_write(d, 0x10, &v, 1);            // CTRL1_XL
...
{
    uint8_t r;
    dev_read(d, 0x10, &r, 1);  printf("CTRL1_XL=0x%02X\r\n", r);
    dev_read(d, 0x12, &r, 1);  printf("CTRL3_C =0x%02X\r\n", r);
    dev_read(d, 0x13, &r, 1);  printf("CTRL4_C =0x%02X\r\n", r);
}

static void cfg_output_high(GPIO_TypeDef* port, uint16_t pin){
  GPIO_InitTypeDef gi = {0};
  gi.Pin = pin; gi.Mode = GPIO_MODE_OUTPUT_PP; gi.Pull = GPIO_NOPULL; gi.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(port, &gi);
  HAL_GPIO_WritePin(port, pin, GPIO_PIN_SET);
}

/* Keep other SPI2 slaves deselected so only IIS3DWB talks */
static void Bus_Quiesce_All_CS(void){
  __HAL_RCC_GPIOF_CLK_ENABLE();
  __HAL_RCC_GPIOH_CLK_ENABLE();
  cfg_output_high(CS_DWB_GPIO_Port, CS_DWB_Pin);           // PF12 HIGH (IIS3DWB)
  cfg_output_high(CS_DHCX_GPIO_Port, CS_DHCX_Pin);         // PH15 HIGH (ISM330DHCX)
  cfg_output_high(CS_IIS2DLPC_GPIO_Port, CS_IIS2DLPC_Pin); // PH6  HIGH (IIS2DLPC) - harmless if absent
}
...
int main(void)
{
  ...
  Bus_Quiesce_All_CS();
  // Bind devices on SPI2 (local to main)
  // Bind devices on SPI2 (local to main) — define BEFORE first use
  //static spi_dev_t DWB = { .spi=&hspi2, .cs_port=CS_DWB_GPIO_Port,  .cs_pin=CS_DWB_Pin  };
  uint8_t who = 0;
  dev_read(&DWB, 0x0F, &who, 1);
  printf("WHO_AM_I = 0x%02X\r\n", who);

  iis3dwb_start(&DWB);
  iis3dwb_dump_regs(&DWB);     // <-- confirm writes stuck (expect CTRL1_XL=0xA0, CTRL3_C has BDU|IF_INC)
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  uint8_t st = 0, raw[6] = {0};
	  dev_read(&DWB, 0x1E, &st, 1);        // STATUS
	  dev_read(&DWB, 0x28, raw, 6);        // OUTX_L..OUTZ_H
	  printf("STATUS=0x%02X  RAW=%02X %02X %02X %02X %02X %02X\r\n",
	         st, raw[0],raw[1],raw[2],raw[3],raw[4],raw[5]);
...
static void MX_GPIO_Init(void)
{
  ...
  //HAL_GPIO_WritePin(GPIOF, CS_DWB_Pin, GPIO_PIN_SET);
...
void HAL_SPI_TxRxCpltCallback(SPI_HandleTypeDef *hspi){
  if (hspi == &hspi2) {
    if (spi_dma_dev) { HAL_GPIO_WritePin(spi_dma_dev->cs_port, spi_dma_dev->cs_pin, GPIO_PIN_SET); }
    spi_dma_done = 1;
  }
}
...
// Mode-0 DMA read: sends [0x80|reg, 0..0] and returns rx[1..]
HAL_StatusTypeDef dev_read_dma(const spi_dev_t *d, uint8_t reg, uint8_t *data, uint16_t len)
{
  if (len + 1 > MAX_XFER) return HAL_ERROR;
  if (hspi2.State != HAL_SPI_STATE_READY) return HAL_BUSY;

  spi_tx[0] = (uint8_t)(0x80 | (reg & 0x7F));
  memset(&spi_tx[1], 0x00, len);

  // select device & remember it
  cs_low(d);
  spi_dma_dev = d;
  spi_dma_done = 0;
  ...
  // wait (simple blocking for now; could use RTOS/semaphore)
  while (spi_dma_done == 0) { /* spin */ }

  if (spi_dma_done == 2) return HAL_ERROR; // error path already deasserted CS in callback

  memcpy(data, &spi_rx[1], len); // skip dummy
  return HAL_OK;
}
