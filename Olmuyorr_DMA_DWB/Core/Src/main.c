...
// Define sensor registers and important values
#define IIS3DWB_WHO_AM_I_ADDR       0x0F
#define IIS3DWB_WHO_AM_I_VAL        0x7B
#define IIS3DWB_CTRL1_XL_ADDR       0x10
#define IIS3DWB_OUTX_L_A_ADDR       0x28

// SPI Read/Write flags
#define READ_FLAG                   0x80 // MSB set to 1 for read operations
...
// Buffers for DMA transfer (1 byte command + 6 bytes data)
uint8_t spi_tx_buf[7];
uint8_t spi_rx_buf[7];

// Flag to indicate when DMA transfer is complete
// 'volatile' is crucial because it's modified by an interrupt
volatile uint8_t spi_data_ready_flag = 0;

// Buffer for UART messages
char uart_buf[100];
// --- NEW DEBUG VARIABLES ---
volatile uint8_t dbg_who_am_i = 0;              // Stores the result of the WHO_AM_I read
volatile HAL_StatusTypeDef dbg_spi_status;      // Stores the return status of the last DMA call
volatile uint32_t dbg_dma_callbacks = 0;        // Counts how many times the DMA callback has run
volatile int16_t dbg_raw_x, dbg_raw_y, dbg_raw_z; // Stores the latest raw sensor data
/* USER CODE END PV */
