...
def handle_pnpl(sender, data: bytes):
    if len(data) % 12 == 0 and len(data) > 0:
        for i in range(0, len(data), 12):
            gx, gy, gz, ax, ay, az = struct.unpack("<hhhhhh", data[i:i+12])
            print(f"[PNPL] G={gx:6d} {gy:6d} {gz:6d} | A={ax:6d} {ay:6d} {az:6d}")
    else:
        print(f"[PNPL] len={len(data)} hex={data.hex()}")
        ...
        async with BleakClient(dev, timeout=45, **kwargs) as client:
          ...
          #  try:
           #     await client.start_notify(BAT_CHAR, handle_battery)
            ...
            try:
                await client.start_notify(PNPL_CHAR, handle_pnpl)
                print("\r\n[INFO] Subscribed to PNPL\r\n")
            except Exception as e:
                print("[WARN] Could not subscribe to PNPL:", e)
