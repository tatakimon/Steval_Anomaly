...
#define DBG_SNAP_BYTES   16   /* keep a small window of what went over the wire */

/* Expectation helper for read commands: MSB=1, addr in low 7 bits */
#define SPI_READ_CMD_MASK   0x80u


#define DWB_SENS_4G_UG_PER_LSB  122   /* 0.122 mg/LSB = 122 µg/LSB */

/* --- IIS3DWB registers --- */
#define DWB_REG_CTRL1_XL    0x10
#define DWB_REG_CTRL3_C     0x12
#define DWB_REG_CTRL4_C     0x13
#define DWB_REG_STATUS      0x1E
#define DWB_REG_OUTX_L_A    0x28
#define DWB_REG_OUTX_H_A    0x29
#define DWB_REG_OUTY_L_A    0x2A
#define DWB_REG_OUTY_H_A    0x2B
#define DWB_REG_OUTZ_L_A    0x2C
#define DWB_REG_OUTZ_H_A    0x2D

/* CTRL1_XL fields: FS[1:0] → ±2/±16/±4/±8 g (00/01/10/11), XL_EN[2:0]=101 */
#define DWB_FS_2G           0x00
#define DWB_FS_16G          0x01
#define DWB_FS_4G           0x02
#define DWB_FS_8G           0x03

#define DWB_REG_WHO_AM_I      0x0F
#define DWB_WHO_AM_I_VALUE    0x7B

/* mg/LSB at 25°C from datasheet */
#define DWB_SENS_2G_g       (0.061f/1000.0f)
#define DWB_SENS_4G_g       (0.122f/1000.0f)
#define DWB_SENS_8G_g       (0.244f/1000.0f)
#define DWB_SENS_16G_g      (0.488f/1000.0f)

/* ST sensor SPI command bits */
#define SPI_READ_BIT          0x80u   /* bit7 = 1 for read */
#define SPI_AUTOINC_BIT       0x40u   /* bit6 = 1 for auto-increment */

#define SPI_DMA_MAX           512     /* DMA buffer size */


/* CTRL1_XL fields (DS12569): [7:5]=XL_EN, [3:2]=FS, [1]=LPF2_XL_EN */
#define DWB_XL_EN_3AXIS     (0b101u << 5)       /* normal mode */
#define DWB_FS_2G           (0u << 2)
#define DWB_FS_4G           (2u << 2)
#define DWB_FS_8G           (3u << 2)           /* per table */
#define DWB_FS_16G          (1u << 2)
#define DWB_LPF2_EN         (1u << 1)

/* CTRL3_C bits */
#define DWB_BDU             (1u << 6)
#define DWB_IF_INC          (1u << 2)
#define DWB_SW_RESET        (1u << 0)
#define DWB_SIM_3WIRE       (1u << 3)           /* leave 0 for 4-wire */

/* CTRL4_C bits */
#define DWB_I2C_DISABLE     (1u << 2)

/* CTRL6_C bits */
#define DWB_XL_AXIS_SEL_3   (0u)                /* 00 = XYZ */

/* CTRL8_XL bits */
#define DWB_FDS             (1u << 2)           /* 0=LP, 1=HP path */
#define DWB_FAST_SETTLE     (1u << 3)
#define DWB_HPCF_MASK       (0b111u << 5)
...
volatile uint8_t  dbg_who = 0;
volatile uint8_t  dbg_status_last = 0;
volatile uint8_t  dbg_out_burst[6] = {0};
volatile HAL_StatusTypeDef dbg_last_st = HAL_OK;

static int32_t ug_per_lsb = DWB_SENS_4G_UG_PER_LSB;
static uint8_t spi_tx_buf[SPI_DMA_MAX];
static uint8_t spi_rx_buf[SPI_DMA_MAX];
static volatile uint8_t spi2_dma_done = 0;
static float g_per_lsb = DWB_SENS_4G_g;  /* matches FS = ±4 g below */

/* SPI/CS snapshots you can watch in Live Expressions/Debugger */
volatile uint8_t  dbg_cs_level = 1;             /* 1=High (idle), 0=Low */
volatile uint8_t  dbg_last_cmd = 0;             /* last SPI command byte sent */
volatile uint8_t  dbg_last_reg = 0;             /* start register for last op */
volatile uint16_t dbg_last_len = 0;             /* payload length of last op */
volatile uint8_t  dbg_tx_head[DBG_SNAP_BYTES];  /* first TX bytes (cmd + dummies/payload) */
volatile uint8_t  dbg_rx_head[DBG_SNAP_BYTES];  /* first RX bytes (dummy + data...) */

volatile uint8_t  dbg_status = 0;               /* last STATUS register value */
volatile uint8_t  dbg_ctrl1  = 0;               /* last CTRL1_XL readback */
volatile uint8_t  dbg_ctrl3  = 0;               /* last CTRL3_C readback */
volatile uint8_t  dbg_ctrl4  = 0;               /* last CTRL4_C readback */

volatile int16_t  dbg_ax = 0, dbg_ay = 0, dbg_az = 0;   /* last raw axes */
volatile int      dbg_spi2_dma_done_seen = 0;           /* toggles when DMA finishes */
volatile uint8_t  dbg_cmd_ok = 0;                        /* (dbg_last_cmd == 0x80 | dbg_last_reg) */
...
static void dbg_update_regs(void);
static void dbg_probe_once(void);   /* optional, call only when you want a one-shot sanity probe */
...
static HAL_StatusTypeDef iis3dwb_write(uint8_t reg, const uint8_t *src, uint16_t len);
static HAL_StatusTypeDef iis3dwb_read_blocking(uint8_t reg, uint8_t *dst, uint16_t len);

static void iis3dwb_reach_test(void);
static void iis3dwb_init_sequence(void);

static void     iis3dwb_config_startup(void);
static uint8_t  iis3dwb_data_ready(void);
static HAL_StatusTypeDef iis3dwb_read_axes(int16_t *ax, int16_t *ay, int16_t *az);


static void     iis3dwb_readback_cfg(void);
static void     iis3dwb_set_scale_from_fs(uint8_t fs_bits);

static uint8_t  iis3dwb_rd8(uint8_t reg);   /* <— prototype added */
...
static void iis3dwb_set_scale_from_fs(uint8_t fs_bits)
{
  switch ((fs_bits >> 2) & 0x03u) {
    case 0: g_per_lsb = DWB_SENS_2G_g;  break;
    case 1: g_per_lsb = DWB_SENS_16G_g; break;
    case 2: g_per_lsb = DWB_SENS_4G_g;  break;
    case 3: g_per_lsb = DWB_SENS_8G_g;  break;
  }
}


/* Strong, verified bring-up per datasheet */
static void iis3dwb_config_startup(void)
{
  /* 0) Soft reset (bit clears itself) */
  uint8_t v = DWB_SW_RESET;
  (void)iis3dwb_write(DWB_REG_CTRL3_C, &v, 1);
  HAL_Delay(2);

  /* 1) BDU=1, IF_INC=1, 4-wire SPI */
  v = (uint8_t)(DWB_BDU | DWB_IF_INC);
  (void)iis3dwb_write(DWB_REG_CTRL3_C, &v, 1);

  /* 2) SPI-only (disable I2C) */
  v = DWB_I2C_DISABLE;
  (void)iis3dwb_write(DWB_REG_CTRL4_C, &v, 1);



  /* 5) Enable accelerometer: XL_EN=101, FS=±4 g, LPF2 disabled */
  v = (uint8_t)(DWB_XL_EN_3AXIS | DWB_FS_4G /*| 0 if LPF2 off*/);
  (void)iis3dwb_write(DWB_REG_CTRL1_XL, &v, 1);

  /* 6) Read back & adopt exact FS for correct scaling */
  uint8_t ctrl1 = iis3dwb_rd8(DWB_REG_CTRL1_XL);
  iis3dwb_set_scale_from_fs(ctrl1);
...
/* Read a byte (reuse your DMA helper; fall back to blocking if needed) */
/* Safe single-byte read (tries DMA helper first, falls back to blocking) */
static uint8_t iis3dwb_rd8(uint8_t reg)
{
  uint8_t v = 0;
  if (iis3dwb_read(reg, &v, 1) != HAL_OK) {
    (void)iis3dwb_read_blocking(reg, &v, 1);
  }
  return v;
}


/* Optional: print current config (helps sanity-check) */
static void iis3dwb_readback_cfg(void)
{
  uint8_t c1 = iis3dwb_rd8(DWB_REG_CTRL1_XL);
  uint8_t c3 = iis3dwb_rd8(DWB_REG_CTRL3_C);
  uint8_t c4 = iis3dwb_rd8(DWB_REG_CTRL4_C);

  printf("CTRL1_XL=0x%02X (FS[3:2]=%u%u)\r\n", c1, (c1>>3)&1, (c1>>2)&1);
  printf("CTRL3_C =0x%02X (BDU=%u IF_INC=%u)\r\n", c3, (c3>>6)&1, (c3>>2)&1);
  printf("CTRL4_C =0x%02X (I2C_disable=%u)\r\n", c4, (c4>>2)&1);

  dbg_update_regs();   /* snapshots STATUS/CTRLx into dbg_* so you can watch them */
}




/* Overwrite your previous iis3dwb_init_sequence with this one */
static void iis3dwb_init_sequence(void)
{
  CS_DWB_High();
  HAL_Delay(5);

  iis3dwb_reach_test();          /* WHO_AM_I check */
  iis3dwb_config_startup();      /* strong config */
  iis3dwb_readback_cfg();        /* print what stuck */
}
...
/* IIS3DWB SPI write via DMA */
static HAL_StatusTypeDef iis3dwb_write(uint8_t reg, const uint8_t *src, uint16_t len)
{
  if (len + 1 > SPI_DMA_MAX) return HAL_ERROR;

  uint8_t cmd = (uint8_t)(reg | (len > 1 ? SPI_AUTOINC_BIT : 0)); /* write: bit7=0 */
  spi_tx_buf[0] = cmd;
  memcpy(&spi_tx_buf[1], src, len);

  spi2_dma_done = 0;
  CS_DWB_Low();
  HAL_StatusTypeDef st = HAL_SPI_Transmit_DMA(&hspi2, spi_tx_buf, len + 1);
  if (st != HAL_OK) { CS_DWB_High(); return st; }
...
/* Optional blocking read for bring-up fallback */
static HAL_StatusTypeDef iis3dwb_read_blocking(uint8_t reg, uint8_t *dst, uint16_t len)
{
	uint8_t cmd = (uint8_t)(0x80u | (reg & 0x7Fu));  //
  CS_DWB_Low();
  HAL_StatusTypeDef st = HAL_SPI_Transmit(&hspi2, &cmd, 1, HAL_MAX_DELAY);
  if (st == HAL_OK) {
    st = HAL_SPI_Receive(&hspi2, dst, len, HAL_MAX_DELAY);
  }
  CS_DWB_High();
  return st;
}

/* Quick reach test: read WHO_AM_I and print it */
static void iis3dwb_reach_test(void)
{
  uint8_t who = 0x00;

  if (iis3dwb_read(DWB_REG_WHO_AM_I, &who, 1) != HAL_OK) {
    uart_print("DMA read failed, trying blocking...\r\n");
    if (iis3dwb_read_blocking(DWB_REG_WHO_AM_I, &who, 1) != HAL_OK) {
      uart_print("Blocking read also failed.\r\n");
      return;
    }
  }

  printf("IIS3DWB WHO_AM_I = 0x%02X\r\n", who);
  if (who != DWB_WHO_AM_I_VALUE) {
    uart_print("Unexpected WHO_AM_I (expect 0x7B). If 0xFF, check CS or SPI mode.\r\n");
  } else {
    uart_print("IIS3DWB reachable over SPI2 (Mode 0, presc 32). ✅\r\n");
  }
}



static void iis3dwb_set_scale(uint8_t fs_code)
{
  switch (fs_code & 0x03) {
    case DWB_FS_2G:  g_per_lsb = DWB_SENS_2G_g;  break;
    case DWB_FS_4G:  g_per_lsb = DWB_SENS_4G_g;  break;
    case DWB_FS_8G:  g_per_lsb = DWB_SENS_8G_g;  break;
    case DWB_FS_16G: g_per_lsb = DWB_SENS_16G_g; break;
  }
}
...
/* Update cached regs (call after your existing readbacks) */
static void dbg_update_regs(void)
{
  /* use your existing helpers to read back current values safely */
  (void)iis3dwb_read(DWB_REG_STATUS, &dbg_status, 1);
  (void)iis3dwb_read(DWB_REG_CTRL1_XL, &dbg_ctrl1, 1);
  (void)iis3dwb_read(DWB_REG_CTRL3_C,  &dbg_ctrl3, 1);
  (void)iis3dwb_read(DWB_REG_CTRL4_C,  &dbg_ctrl4, 1);
}

/* OPTIONAL: one-shot probe to compare burst vs single-byte reads */
static void dbg_probe_once(void)
{
  uint8_t single[6] = {0}, burst[6] = {0};

  /* six single reads 0x28..0x2D */
  for (uint8_t i=0;i<6;i++) {
    (void)iis3dwb_read((uint8_t)(DWB_REG_OUTX_L_A + i), &single[i], 1);
  }
  /* one burst read starting at 0x28 */
  (void)iis3dwb_read(DWB_REG_OUTX_L_A, burst, 6);

  /* keep in rx_head for easy viewing */
  for (int i=0;i<6;i++) dbg_rx_head[i] = burst[i];
...
/* ---- tap into your existing IIS3DWB I/O functions (ADD the lines marked DBG) ---- */

/* In your iis3dwb_read(...) just BEFORE calling HAL_SPI_TransmitReceive_DMA add: */
/// dbg_clear_snap(); dbg_capture_tx(spi_tx_buf, len+1, reg, spi_tx_buf[0]);  /* DBG */

/* and just AFTER DMA completes (before memcpy to dst) add: */
/// dbg_capture_rx(spi_rx_buf, len+1);  /* DBG */

/* In your iis3dwb_read_axes(...) AFTER unpacking ax,ay,az add: */
/// dbg_ax = *ax; dbg_ay = *ay; dbg_az = *az;  /* DBG */

/* In your init sequence, AFTER you print the CTRLx values, add: */
/// dbg_update_regs();  /* DBG */
