...
extern UART_HandleTypeDef huart2; // we print on USART2
...
// Block-level stats (every 50 windows)
#define BLOCK_SIZE 50   // number of windows per summary block


static uint32_t block_win_count = 0;      // how many windows processed in current block
static uint32_t block_N_count   = 0;      // N states in current block
static uint32_t block_A_count   = 0;      // A states in current block
static uint32_t block_start_id  = 0;      // win_id of first window in this block
...
#define WIN_SIZE 		   400  /* 250 samples per window */
#define DECIM_FACTOR       7        // downsample from ~7 kHz -> ~1 kHz
#define BASELINE_WINDOWS   100      // ~30 s of training
...
static void process_window(int64_t E2, int32_t mx, int32_t my, int32_t mz)
{
    ...
    // C = calibrating, N = normal, A = anomaly

        // ... your existing calibration / detection / LED logic ...
        // at this point, for DET_RUNNING, state_char is 'N' or 'A'
        // and win_id still refers to THIS window number
    ...
    if (det_state == DET_RUNNING) {
        ...
        if (block_win_count == 0) {
           	            // first window in this block
           	            block_start_id = win_id;
           	        }
           	     ...
           	     if (state_char == 'N') {
           	                 block_N_count++;
           	             } else if (state_char == 'A') {
           	                 block_A_count++;
           	             }


           	  if (block_win_count >= BLOCK_SIZE) {
           	            uint32_t block_end_id = win_id;
           	         char stats_line[128];
           	                     int n2 = snprintf(stats_line, sizeof(stats_line),
           	                                       "BLOCK %lu-%lu total=%lu N=%lu A=%lu\r\n",
           	                                       (unsigned long)block_start_id,
           	                                       (unsigned long)block_end_id,
           	                                       (unsigned long)block_win_count,
           	                                       (unsigned long)block_N_count,
           	                                       (unsigned long)block_A_count);
           	                     if (n2 > 0 && n2 < (int)sizeof(stats_line)) {
           	                         HAL_UART_Transmit(&huart2, (uint8_t *)stats_line, n2, HAL_MAX_DELAY);
           	                     }
